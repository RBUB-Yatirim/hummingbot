...
from hummingbot.connector.exchange.btcturk.btcturk_utils import convert_to_exchange_trading_pair, convert_from_exchange_trading_pair

BTCTURK_REST_URL = "https://api.btcturk.com"
DIFF_STREAM_URL = "wss://ws-feed-pro.btcturk.com/"
TICKER_PRICE_CHANGE_URL = "https://api.btcturk.com/api/v2/ticker"
SINGLE_MARKET_DEPTH_URL = "https://api.btcturk.com/api/v2/ticker?pairSymbol={}"
EXCHANGE_INFO_URL = "https://api.btcturk.com/api/v2/ticker"
ORDERBOOK_REST_URL = "https://api.btcturk.com/api/v2/orderbook?pairSymbol={}&limit={}"
...
class BtcturkAPIOrderBookDataSource(OrderBookTrackerDataSource):
    ...
    async def get_last_traded_prices(cls, trading_pairs: List[str]) -> Dict[str, float]:
        async with aiohttp.ClientSession() as client:
            ...
            return {convert_from_exchange_trading_pair(market): float(data["pair"]["last"]) for market, data in resp_json['data]'.items()
    ...
    def get_mid_price(trading_pair: str) -> Optional[Decimal]:
        ...
        try:
            resp = requests.get(url=f"{SINGLE_MARKET_DEPTH_URL.format(exchange_trading_pair)}").json()
            best_bid: Decimal = Decimal(resp["data"][0]['bid'])
            best_ask: Decimal = Decimal(resp["data"][0]['ask'])
            ...
            async with aiohttp.ClientSession() as client:
                async with client.get(EXCHANGE_INFO_URL, timeout=API_CALL_TIMEOUT) as response:
                    if response.status == 200:
                        ...
                        raw_trading_pairs = [d["pairNormalized"] for d in data['data']]
    ...
    async def get_snapshot(client: aiohttp.ClientSession, trading_pair: str, limit: int = 1000) -> Dict[str, Any]:
        request_url: str = ORDERBOOK_REST_URL.format(convert_to_exchange_trading_pair(trading_pair), limit)
        async with client.get(request_url) as response:
            ...
            return _prepare_snapshot(trading_pair, data['data']["bids"], data["data"]["asks"])
